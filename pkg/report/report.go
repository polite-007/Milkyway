...
func prepareReportData(result *config.AssetsResult) (*ReportData, error) {
	...
	// 计算总资产数量
	totalAssets := 0
	for _, ipInfo := range result.IpInfos {
		totalAssets += len(ipInfo.Ports)
	}
	totalAssets += len(result.UrlInfos) // 加上URL资产

	// 计算总漏洞数量
	totalVulns := 0
	for _, ipInfo := range result.IpInfos {
		totalVulns += len(ipInfo.ProtocolVul) + len(ipInfo.WebVul)
	}
	for _, urlInfo := range result.UrlInfos {
		totalVulns += len(urlInfo.WebVul)
	}
...
func calculateHighRiskVulns(result *config.AssetsResult) int {
	...
	// 统计URL相关的Web漏洞
	for _, urlInfo := range result.UrlInfos {
		for _, vul := range urlInfo.WebVul {
			if vul.Level == "高危" {
				count++
			}
		}
	...
	// 统计协议漏洞
	for _, ipInfo := range result.IpInfos {
		for _, vul := range ipInfo.ProtocolVul {
...
func processIPList(result *config.AssetsResult) ([]IPData, error) {
	...
	for _, ipInfo := range result.IpInfos {
		...
		// 收集该IP的所有端口
		ipData.Ports = collectPortsFromIpInfo(ipInfo)

		// 检查Web服务
		ipData.HasWeb = checkWebServiceFromIpInfo(ipInfo)
		if ipData.HasWeb {
			ipData.Type = "Web"
		}

		// 收集漏洞信息
		ipData.Vulns = collectVulnsFromIpInfo(ipInfo)
		ipData.HasVul = len(ipData.Vulns) > 0

		// 收集Web服务信息
		ipData.WebInfos = collectWebInfosFromIpInfo(ipInfo)

		ipList = append(ipList, ipData)
	}

	// 处理URL信息（将URL信息转换为IP格式显示）
	for _, urlInfo := range result.UrlInfos {
		// 从URL中提取IP或域名
		if urlInfo.WebInfo == nil {
			continue
		}
		host := urlInfo.WebInfo.Url.Hostname()

		// 检查是否已经存在该主机的IP数据
		existingIPData := findIPDataByHost(ipList, host)
		if existingIPData != nil {
			// 如果存在，将URL信息添加到现有IP数据中
			webInfo := convertRespToWebInfo(urlInfo.WebInfo)
			existingIPData.WebInfos = append(existingIPData.WebInfos, webInfo)

			// 添加URL的漏洞信息
			for _, vul := range urlInfo.WebVul {
				vulnInfo := convertWebVulToVulnInfo(vul)
				existingIPData.Vulns = append(existingIPData.Vulns, vulnInfo)
			}
			existingIPData.HasVul = len(existingIPData.Vulns) > 0
		} else {
			// 如果不存在，创建新的IP数据
			ipData := IPData{
				IP:   host,
			...
			for _, vul := range urlInfo.WebVul {
				vulnInfo := convertWebVulToVulnInfo(vul)
				ipData.Vulns = append(ipData.Vulns, vulnInfo)
			}
			ipData.HasVul = len(ipData.Vulns) > 0
...
// collectPortsFromIpInfo 从IpInfo中收集端口信息
func collectPortsFromIpInfo(ipInfo *config.IpInfos) []PortInfo {
	var ports []PortInfo
	if ipInfo == nil {
		return ports
	}
...
// checkWebServiceFromIpInfo 检查IpInfo是否有Web服务
func checkWebServiceFromIpInfo(ipInfo *config.IpInfos) bool {
	if ipInfo == nil {
		return false
	}

	for _, port := range ipInfo.Ports {
		if port != nil && (port.Protocol == "http" || port.Protocol == "https") {
			return true
		}
	}
	return false
...
// collectVulnsFromIpInfo 从IpInfo中收集漏洞信息
func collectVulnsFromIpInfo(ipInfo *config.IpInfos) []VulnInfo {
	var vulns []VulnInfo
	...
	for _, vul := range ipInfo.ProtocolVul {
		if vul != nil {
			vulns = append(vulns, VulnInfo{
				...
				URL:         fmt.Sprintf("%s://%s:%d", vul.Protocol, ipInfo.IP, vul.Port),
...
// collectWebInfosFromIpInfo 从IpInfo中收集Web服务信息
func collectWebInfosFromIpInfo(ipInfo *config.IpInfos) []WebInfo {
...
// findIPDataByHost 根据主机名查找IP数据
func findIPDataByHost(ipList []IPData, host string) *IPData {
	for i := range ipList {
		if ipList[i].IP == host {
			return &ipList[i]
		}
...
// convertRespToWebInfo 将Resp转换为WebInfo
func convertRespToWebInfo(resp *config.Resp) WebInfo {
	webInfo := WebInfo{
		URL:        resp.Url.String(),
		Title:      resp.Title,
		Cms:        resp.Cms,
...
// convertWebVulToVulnInfo 将WebVul转换为VulnInfo
func convertWebVulToVulnInfo(vul *config.WebVul) VulnInfo {
	return VulnInfo{
		Type:        "Web",
		Name:        vul.VulName,
		Level:       vul.Level,
