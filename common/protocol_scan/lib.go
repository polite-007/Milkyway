...
func ReadDataSmtp(conn net.Conn) ([]byte, error) {
	var SmtpFirst = make([]byte, 3)
	if _, err := ReadWithTimeout(conn, SmtpFirst, timeout); err != nil {
		return nil, err
	}
	if string(SmtpFirst) != "220" {
		return nil, ErrNoCurrProtocol
	}
	var SmtpTwo = make([]byte, 1024)
	if _, err := ReadWithTimeout(conn, SmtpTwo, timeout); err != nil {
		return nil, err
	}
	if strings.Contains(string(SmtpTwo), "SMTP") {
		return append(SmtpFirst, SmtpTwo...), nil
	} else {
		return nil, ErrNoCurrProtocol
	}
}
